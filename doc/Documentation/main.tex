...
In order to reduce the amount of data that we need to parse and search for reasonable elements, we filter out a lot of tags that we do not want to get at all: Since we are looking or areas or polygons, we do not request any ways or relations that have tags containing route*\todo{make clear that this is a wildcard}, highway, associatedStreet or TMC, since these kind of elements do not provide us with polygon information. We furthermore do not request any boundary tags, since we do not believe that we should mark entire political or administrative areas as the intended areas of SURs. A last tag that we are ignoring, is building:part. According to the OSM wiki, any building part that is tagged with building:part should be contained in another element, that covers the entire building, which we are more interested in.
...
After we have received and parsed the OSM data around the given SUR coordinates, we search for the closest elements. If we know that the SUR is only applicable indoor, we only search for relations and ways containing the building tag. Otherwise we search for any relation and way. Since relations are only usable to us, if they contain some sort of polygon information, and the distance of a relations is equal to the distance of it's closest member, we discard any relations that are further away than the closest ways we find. In order to reduce the risk of finding wrong polygons in areas where there are no detailed OSM elements, we search for ways containing the tag landuse, if the SUR coordinates are not inside the closest way and the closest way is further away than an experimentally set threshold of roughly 16m. We use the closest landuse instead of the previously found ways if the SUR coordinates are inside the landuse. This allows us, for example, to find the residential area for SURs whose intended area is a building that is not present in the OSM data. 
...
Any ways we found will also be added to the list of possible solutions unless they are tagged as landuse.
Landuse ways are analysed further: If the way should be used to mark a residential area, we search for buildings within the outline of the landuse. This is done because most buildings are inside the landuse instead of on the border, which prevented us from finding them when searching for the closest ways before. If the closest buildings from within the landuse are closer than the experimentally determined distance of roughly 8m we use the ways marking these buildings instead of the landuse. If we do not find a suitable building we keep the landuse but store that we are uncertain about the quality of such an result, which will be visualized by using a different colour (blue) for the polygon in the kml file. This uncertain colour is also used for other landuse ways such as industrial and commercial, since we do not think that the SUR's area of application is intended to cover such a big area.

If we still have multiple possible ways after filtering through the list of closest elements, we continue to filter out ways that seem less likely. First we check if one of the possible ways marks a building or shop. If this is the case and the rule classification indicates that the SUR can be applicable indoor, we filter out anything that is not a building because in these cases we assume that it is more likely that the intended area for the SUR covers a building. 
...
A good example for this is the number 0038 of the provided test dataset, that is shown in figure \ref{fic:38}: The intended area is a building which is however not mapped in OpenStreetMap yet. Since there is no information about the intended polygon area in the OSM data, we cannot build the polygon. In this case we find the entire residential block and mark it with a different colour to show our uncertainty. \todol{Another example for insufficient information is that the received data does not include parts of a buildings. This is the reason we only find the whole building and not the intended part of the truth files.} Once the OSM data gets updated further we would identify the correct area as well.
...
\todol{give tabular a caption}
\todol{say sth about missing data}
\begin{table}[h]
\begin{tabular}{|r|c|c|c|c|}
\hline 
 & calculated=truth & calculated intuitively correct & calculation uncertain & calculation unintuitive \\ 
\hline 
given SURs & 44,16\% & 21,12\% & 13,44\% & 21,28\% \\ 
\hline 
own SURs & 75\% & 0\% & 25\% & 0\% \\ 
\hline 
\end{tabular} 
...
\todol{restructure - this looks ugly}
...
\todol{Try using the actual area or the area of the bounding box of a polygon instead of the average vertex distance.}
